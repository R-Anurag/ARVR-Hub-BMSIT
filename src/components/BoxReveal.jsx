...
export function BoxRevealDemo() {
  ...
  return (
    <div className="size-full max-w-lg flex flex-col items-start justify-center overflow-hidden font-geistsans">
      {/* Section Title */}
      <BoxReveal boxColor={"#5046e6"} duration={0.5}>
        <p className="text-[3.5rem] font-orbitron">Who Are We?</p>
      </BoxReveal>

      {/* Subtitle */}
      <BoxReveal boxColor={"#5046e6"} duration={0.5}>
        <p className="text-lg text-[#B48CDE] mt-[0.5rem] text-[1rem]">
      ...
      <div className="mt-6 flex flex-col gap-1.5">
        {techHighlights.map((line, index) => (
          <motion.p
            key={index}
            className="text-[0.95rem] text-gray-300 flex items-center gap-2 font-geistmono"
            custom={index}
            initial="hidden"
            animate="visible"
      ...
      <BoxReveal boxColor={"#5046e6"} duration={0.5}>
        <NavLink to="/Teams">
          <Button className="mt-[1.6rem] bg-[#5046e6] text-lg font-geistsans">
            See the Team
          </Button>
        </NavLink>
      </BoxReveal>
